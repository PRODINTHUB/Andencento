...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        sys.modules["ULTRA.utils"] = Speedo.utils
        sys.modules["userbot.Config"] = Speedo.config
        sys.modules["userbot.uniborConfig"] = Speedo.config
        sys.modules["ub"] = Speedo
        sys.modules["var"] = Speedo.variables
        sys.modules["jarvis"] = Speedo
        sys.modules["support"] = Speedo
        sys.modules["userbot"] = Speedo
        sys.modules["telebot"] = Speedo
        sys.modules["fridaybot"] = Speedo
        sys.modules["jarvis.utils"] = Speedo.utils
        sys.modules["uniborg.util"] = Speedo.utils
        sys.modules["telebot.utils"] = Speedo.utils
        sys.modules["userbot.utils"] = Speedo.utils
        sys.modules["userbot.events"] = Speedo.utils
        sys.modules["jarvis.jconfig"] = Speedo.config
        sys.modules["userbot.config"] = Speedo.config
        sys.modules["fridaybot.utils"] = Speedo.utils
        sys.modules["fridaybot.Config"] = Speedo.config
        sys.modules["userbot.uniborgConfig"] = Speedo.config
        ...
        sys.modules["userbot.mainfiles.events"] = Speedo.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Speedo.plugins." + shortname] = mod
        LOGS.info("Speedo imported " + shortname)
...
def on(**args):
    def decorator(func):
        ...
        speedo.add_event_handler(wrapper, events.NewMessage(**args))
        return wrapper
...
async def is_admin(speedo, chat_id, user_id):
    if not str(chat_id).startswith("-100"):
        return False
    try:
        req_jo = await speedo(GetParticipantRequest(channel=chat_id, user_id=user_id))
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Speedo/assistant/{shortname}.py")
        name = "Speedo.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
