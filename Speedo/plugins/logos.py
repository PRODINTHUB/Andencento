...
async def lg1(speedoevent):
    ...
    if speedoevent.reply_to_msg_id:
        ...
        async for i in bot.iter_messages("@SPEEDOBOT_LOGOS", filter=InputMessagesFilterPhotos):
    	    PICS_STR.append(i)
        pic = random.choice(PICS_STR)
        logo_ = await pic.download_media()
    text = speedoevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    await bot.send_file(
        speedoevent.chat_id,
        file_name,
        caption=f"**Made By :** {speedo_mention}",
    )
    await event.delete()
    try:
        os.remove(file_name)
