...
async def _(speedoevent):
    ...
    if not speedoevent.reply_to_msg_id:
       await eod(speedoevent, "`Please Reply To A User To Get This Module Work`")
       return
    reply_message = await speedoevent.get_reply_message() 
    chat = "Sangmatainfo_bot"
    victim = reply_message.sender.id
    if reply_message.sender.bot:
       await eod(speedoevent, "Need actual users. Not Bots")
       return
    ...
    async with speedoevent.client.conversation(chat) as conv:
          try:     
              ...
              await eod(speedoevent, "Please unblock @Sangmatainfo_bot")
              return
