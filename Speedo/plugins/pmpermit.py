...
SPEEDO_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
SPEEDO_FIRST = (
...
if PM_ON_OFF != "DISABLE":
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(SPEEDO_ZERO)
            await asyncio.sleep(3)
...
CmdHelp("pm_permit").add_command(
  ...
  "listapproved", None, "Sends the list of all users approved by SPEEDOBOT"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
